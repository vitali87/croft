...
└── widgets/
    ...
    ├── extensions.rs    Extensions sidebar widget: projects the bundled + user extension manifests (lsp/manifest.rs summaries) into a toggleable list with per-row Enabled/Disabled state; reports row clicks + action-pill clicks back to App, which owns the disabled-set prefs and the feature gating
