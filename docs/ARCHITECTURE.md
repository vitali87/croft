...
├── voice.rs             push-to-talk voice input for the Termux OSK mic key: delegates to `termux-speech-to-text` (Android's SpeechRecognizer, the same engine Gboard's mic uses), auto-installs the `termux-api` package (InstallState), and streams the transcript over a channel the app injects via handle_key; mic press starts, release kills the client and inserts the recognized text once (final-only, since upstream buffers true partials)
