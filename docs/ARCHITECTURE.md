...
**Symbol tabs.** `Cmd+K V` (palette "Open Symbol in Its Own Tab", OUTLINE right-click, `Alt+Enter` in the workspace symbol picker) opens a normal `Editor` tab that holds the whole file plus a `symbol_view: Option<SymbolView>` (`src/symbol_range.rs`). Render, caret, scroll, edge deletes, line moves, join, counted line deletes, find and Replace All are clipped to the view's `first..=last` lines, so every editor feature works unchanged. Each tick `App::sync_symbol_views` mirrors the text between all tabs of one path (the tab whose `edit_seq` moved past its `mirror_seq` is the source; a line-level splice via `Editor::mirror_lines_from` keeps carets and clear selections, and folds each of the source's undo steps (`undo_step_id`) into one step of its own) and runs again right after every editor key and before the watcher drain, so a save from one tab never reads as a conflict in another unless the file is live in a collab session, then `SymbolView::follow` shifts the range through the edit (an edit on the range's edge is placed by the caret that made it: a line opened just above a symbol stays out, one opened at its end from its own tab grows it), re-derives the name from the Tree-sitter outline when the first line changed, and re-anchors to a same-named symbol of the same kind overlapping the old range when it is lost; a symbol that is gone closes its tab. `find_tab_with_path` skips symbol tabs so opening a file never lands in one; session capture keeps only orphaned symbol tabs, as plain file tabs.
