...
**Symbol tabs.** `Cmd+K Shift+V` (palette "Editor: Open Symbol as Its Own Tab", OUTLINE right-click, `Alt+Enter` in the workspace symbol picker) opens a normal `Editor` tab that holds the whole file plus a `symbol_view: Option<SymbolView>` (`src/symbol_range.rs`). Render, caret, scroll, secondary carets, occurrence selection, edge deletes, line moves, join, counted line deletes, find and Replace All are clipped to the view's `first..=last` lines, so every editor feature works unchanged. Vim operators clamp their lines and selection to those lines too, and the minimap draws and maps only them. A file and its symbol tabs share one group with separate `edit_seq`s, so every per-path pass over `iter_tabs()` must let one tab speak for the path: `sync_lsp` takes the active tab when it holds the file, else the first; `sync_markdown_lint` takes the first. The minimap's re-emit key carries the document and its drawn lines, so switching between two tabs that agree on everything else still repaints it. A symbol tab is found again by its tree-sitter identity (name, kind, depth and item line), not by the picker's title or first line. `App::sync_symbol_views` runs each tick, right after every editor key and before the watcher drain. It first mirrors the text between all tabs of one path, except for a file live in a collab session, whose tabs each receive the session's edits: the tab whose `edit_seq` moved past its `mirror_seq` is the source, and `Editor::mirror_lines_from` splices its lines into the others, keeping carets and clear selections and folding each of the source's undo steps (`undo_step_id`) into one step of its own. Because siblings are synced before any watcher event, a save from one tab never reads as a conflict in another. Then `SymbolView::follow` shifts each range through the edit. An edit on the range's edge is placed by the caret that made it: a line opened just above a symbol stays out, one opened at its end from its own tab grows it, and a collaborator's edit is never the tab's own. The name is re-derived from the Tree-sitter outline only when the edit touches the symbol's heading line, whose offset moves with lines added or removed above it; a lost range re-anchors to a same-named symbol of the same kind and depth overlapping the old lines carried through the edit, and a symbol that is gone closes its tab, unless the tab holds unsaved text no whole-file tab of the path holds: then it becomes that file's ordinary tab, so the edits survive. `find_tab_with_path` skips symbol tabs so opening a file never lands in one; session capture keeps only orphaned symbol tabs, one per file, as plain file tabs.
