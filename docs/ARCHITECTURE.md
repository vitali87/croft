...
├── voice.rs             tap-to-toggle voice input for the Termux OSK mic key: delegates to `termux-speech-to-text` (Android's SpeechRecognizer, the same engine Gboard's mic uses), auto-installs the `termux-api` package (InstallState), and streams the transcript over a channel the app injects via handle_key; one tap starts, a second tap (or end-of-speech) stops and inserts the recognized text once (final-only, since upstream buffers true partials; a tap rather than hold because Termux steals finger-holds for text selection)
