...
└── widgets/
    ...
    ├── extensions.rs    Extensions sidebar widget: projects the bundled + user extension manifests (lsp/manifest.rs summaries) into a theme-aware list — grouped under BUILT-IN/INSTALLED headers, each row a coloured language/file mark + name + blurb + a rounded pill toggle (Powerline caps + knob, brand-teal on / grey off) — plus a local filter box (narrows by name/blurb/id; no marketplace); reports row / switch / clear-button clicks back to App, which owns the disabled-set prefs and the feature gating
