...
**File renames (`workspace/willRenameFiles`, `didRenameFiles`).** The client declares `workspace.fileOperations`, and each server's `willRename` / `didRename` filters compile into globsets at spawn (file scheme only, matched against the whole path, folder or file kind honoured). An Explorer rename, cut-paste, drag or Finder drop is planned first (destinations validated, same-named sources kept apart), then held while any matching server answers `willRenameFiles`, each bounded by a 60 second timeout; `Esc` skips the wait, and a move still held when croft quits or re-roots goes ahead without the servers' edit. The merged edit is applied at the files' old paths (open text tabs in memory as one undo step, closed files on disk), the files move, tabs follow, and `didRenameFiles` reports the moves that happened. With no listening server the move is immediate: a worker-maintained flag avoids the round trip. The same applier serves F2 and quick-fix edits. A copy of the file in another editor group takes the edit only when it holds the text the server computed against (the active group's tab, or else the disk); a diverged copy keeps its text and reads the disk write as an external change, and a diff or hex view never takes it. A file typed into, or written on disk, while the move waits for the servers keeps its own text, and the status names it; a file opened clean during the wait still takes the edit. A closed file counts as unwritten only when its modified time is at least two seconds older than the request (some filesystems round file times to the second), so a file another program wrote just before the move is also left alone and named; croft's own rename edits to closed files are remembered (a hash of the text written, by path, for two seconds) and reported to the servers with `workspace/didChangeWatchedFiles` before any later request, so a second move right after one that rewrote the same file still edits it; one croft wrote after the request went out is named like any other change.
