...
├── comments.rs           human comment boxes (#367): a participant's sticky notes on a line, overlay state on the navigator's comment-box widget; the owner holds the canonical `CommentStore`, persists it per workspace and broadcasts it, and anchors move by commuting `CommentEdit::Shift` deltas sent by the author of the local edit
