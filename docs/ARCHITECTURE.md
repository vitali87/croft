...
│   ├── session.rs            one debug launch session: the initialize → setBreakpoints → configurationDone → stopped state machine, event classifier, the stackTrace → scopes → variables chain, evaluate, breakpoints and logpoints, hit-count, function and data breakpoints gated on the adapter's capabilities, Run to Cursor, over one adapter-agnostic launch_with
