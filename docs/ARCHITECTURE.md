...
├── voice.rs             voice input for the Termux OSK mic key: delegates to `termux-speech-to-text` (Android's SpeechRecognizer, the same engine Gboard's mic uses), auto-installs the `termux-api` package (InstallState), and sends the transcript over a channel the app injects via handle_key; a tap starts, the result is injected when Android finalizes on silence (a second tap cancels, since killing preempts the result), final-only mode; runs in its own process group so cancel kills the whole tree; a tap rather than hold because Termux steals finger-holds for text selection
