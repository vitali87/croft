...
**Drop-relay pump.** The local launcher tails the remote `requests.log` and runs each verb locally: pull, clipboard, open, plus forward (adds an `-L` to the live SSH master via `ssh -O forward` and opens the local browser) and unforward (`ssh -O cancel`).
