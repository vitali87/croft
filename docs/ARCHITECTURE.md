...
**Drop-relay pump.** The local launcher tails the remote `requests.log` and runs each verb locally: pull, clipboard (fetch the local clipboard for a remote paste), copy (the reverse — put a remote croft's copied text on the LOCAL clipboard, so `Cmd+C` on a remote box reaches the clipboard the user pastes from), open, plus forward (adds an `-L` to the live SSH master via `ssh -O forward` and opens the local browser) and unforward (`ssh -O cancel`). Every local window attached to the same remote session runs its own pump over the same log, so a pump first claims a request with a plain `mkdir` of its inbox directory (no `-p`, so exactly one pump wins) and skips requests it lost. Without the claim, one click opened a browser tab per window. `unforward` is not claimed, since cancelling twice is harmless.
