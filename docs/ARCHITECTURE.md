...
└── widgets/
    ...
    ├── extensions.rs    Extensions sidebar widget: projects the bundled + user extension manifests (lsp/manifest.rs summaries) into a list with a per-row toggle switch (blue on / grey off) and a local filter box (narrows by name/blurb/id; no marketplace); reports row / switch / clear-button clicks back to App, which owns the disabled-set prefs and the feature gating
