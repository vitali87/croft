...
**Voice input.** A mic key sits immediately right of the left `alt`, taking its width from the space bar (the left space half in the split layout). Suppressing the native keyboard also removes its mic button, so this restores dictation: **tap the mic to start, tap again to stop** (it also stops on its own when you pause). It is a tap toggle rather than press-and-hold because Termux turns any finger hold on the terminal into its own text-selection gesture, which a TUI cannot suppress. It glows while listening and the status line shows `Listening…`. Under the hood it calls `termux-speech-to-text`, which drives Android's system speech recognizer (the same engine Gboard's mic uses), so the transcript lands wherever the cursor is, the editor, a terminal, or a modal. The first tap installs the `termux-api` package automatically (background `pkg install`); you also need the **Termux:API app** from F-Droid and to grant it microphone permission, since the package alone cannot reach the recognizer. The transcript is inserted once when recognition stops rather than streaming word by word, because the upstream progressive-output path buffers (termux-api-package#137).
