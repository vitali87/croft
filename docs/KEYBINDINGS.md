...
|---------|-------|--------|
| Debug: Select and Start Debugging | palette only | Pick a `launch.json` configuration (or the zero-config "Debug active file" entry) and start it; the pick becomes what `F5` and restart launch |
...
| `@`{a-z} / `@@` (vim Normal) | Replay a register, or repeat the last macro. Counts apply: `3@a` runs it three times. Iterations never merge into each other — each starts a fresh undo step — but a macro containing several kinds of edit still undoes in the steps those edits would make on their own |
