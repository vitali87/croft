...
Beyond the zero-config active file, croft reads **debug configurations** from `.vscode/launch.json` (JSONC tolerated, unchanged from VS Code) and `.croft/launch.json` (same schema, for repos that don't want a `.vscode/` directory; it also accepts a bare configuration array). Mapped fields: `name`, `type` (`python`/`debugpy`, `lldb`/`cppdbg`, `node`/`pwa-node`), `request` (`launch`/`attach`), `program`, `module`, `args`, `env`, `envFile`, `cwd`, `preLaunchTask` (matched against Tasks: Run Task labels; the task runs in its pane and the launch waits for exit 0), `stopOnEntry`, `port` (node and python attach), and `processId` (lldb attach). Every other field passes through to the adapter verbatim, so adapter-specific keys (`justMyCode`, `sourceMaps`, …) work without croft knowing them. `${workspaceFolder}`, `${file}`, `${fileBasename}`, `${fileBasenameNoExtension}`, `${fileDirname}`, `${fileExtname}`, `${cwd}`, `${pathSeparator}`, and `${env:VAR}` substitute; an unresolvable variable fails the launch with a named error instead of running something half-substituted. `console` is forced to `internalConsole` (croft debugs in-process). **Compounds** (`"compounds": [{ "name": …, "configurations": [...] }]`) are read from the same file and listed in that picker with their members; members may be bare names or `{ "folder", "name" }` objects, and a compound naming a configuration no launch.json declares reports that member by name. Selecting a compound that names a SINGLE configuration launches it — that needs one session, which croft runs. One that names several needs several debug sessions at once, which croft does not support yet (#310), so selecting it says so rather than starting a subset. A compound's own `preLaunchTask` is honoured: it runs before the member starts and the launch waits for it to exit 0, chaining with the member's own task if it declares one (VS Code runs both). A single-member compound that sets a `presentation` croft cannot honour is still refused rather than launched without it (#318). Pick a configuration with "Debug: Select and Start Debugging" or the config row under the Run and Debug panel's button; the pick is what `F5` and restart launch until changed, and an edited launch.json re-reads on the next `F5`.
...
| `Cmd+Enter` (Markdown) / click `▷` (preview) | Runnable docs (#353): every `sh`/`bash`/`zsh`/`fish` fence (and `python`/`node` when the interpreter is on PATH) wears a `▷` on its first line in the Markdown preview. Clicking it - or `Cmd+Enter` with the caret in the fence in the source - shows a confirm popup with the whole block, the pane it will run in (`<file>:<n>`) and its directory (the document's, or the workspace root with `{cwd=root}`); `Y` types it there, reusing that pane when it sits idle at a prompt. Every block confirms, because a README is untrusted input; blocks that look destructive (`rm -rf`, `sudo`, `curl … \| sh`, redirects, `nc`, `chmod`, `--force`, `git reset --hard`) or say `{confirm}` turn the popup red. `{run=false}` removes the glyph; a block carrying a control or bidi/zero-width character is never runnable; the pane is named by the document's path under the workspace  After a block finishes, its output appears under the fence in the preview (#354): the last twelve rows, coloured as the pane printed them, with the exit code named when it is not zero, or a note that croft stopped waiting while the block kept running. `{persist}` WRITES that output back into the document as a fence croft replaces on each run, which is the only attribute that touches the file on disk and is off unless asked for; `{timeout=N}` stops the capture after N seconds without stopping the block. |
...
| Agents: Show Changed Files (palette) | The review queue, one row per agent, grouped by the workspace root its files are in (#348): a worktree lane's group is named by its branch (`agent/fix-login 2: other.rs, mod.rs`), any other root by its display label (its folder name, disambiguated by the parent when two roots share it), and anything outside every root by "outside the workspace". A row whose files all sit under one root keeps the flat `agent: n to review (a.rs, b.rs)` shape, and a sample of at most three names ends in `…` when there are more |
