...
|---------|-------|--------|
| Debug: Select and Start Debugging | palette only | Pick a `launch.json` configuration (or the zero-config "Debug active file" entry) and start it; the pick becomes what `F5` and restart launch. Compounds are listed but cannot be launched yet (#310) |
