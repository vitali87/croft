...
| `F12` / `Ctrl`+click | Go to Definition (`Shift`+`Ctrl`+click navigates back); `Ctrl` because mouse reports carry no `Cmd` bit and `Option` belongs to multi-cursor. On a range the language server marked as a link (`textDocument/documentLink`: a URL in a comment, an import specifier), the click follows the link instead — web targets to the system opener (refused with guidance in a remote session), `file://` targets into the editor |
=======
