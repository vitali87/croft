...
**Rendered log (`.log` / `.txt` / `.out` / `.ansi` / extension-less files whose first 8 KiB carry ANSI colour)**

| Keys | Action |
|------|--------|
| Highlighting (palette toggle) | A file opened as a rendered log (a `.log` / `.txt` / `.out` / `.ansi` / extension-less file whose first 8 KiB carry ANSI colour) has its plain lines coloured by [tailspin](https://github.com/bensadeh/tailspin): dates, durations, numbers, UUIDs, IPs, URLs, paths, quotes and severity keywords. Lines that already carry their own colours are left as printed. "Log: Toggle Highlighting (tailspin)" switches it for every open log and for logs opened later, persisted as `disable_log_highlight` in `config.json` (also a row in Settings) |
