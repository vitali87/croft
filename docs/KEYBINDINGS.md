...
| `Cmd+K` `Shift+D` | Session: Detach. This window leaves the persistent session, which keeps running; the terminal is handed back to the shell and the same `croft attach` / `croft remote` command reattaches. The session host answers the chord itself, so it also works for a read-only participant |
...
| Session: Detach | `Cmd`+`K` `Shift`+`D` | Detach this window from the persistent session; the session keeps running and the same command reattaches |
