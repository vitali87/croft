...
| `Cmd+K` `V` | Show the Explorer's AGENT LANE section, open: each coding agent that changed files, then those files (unreviewed first, marked `●`). Clicking a file opens its diff against the reviewed snapshot. The section is also toggled from the EXPLORER ⋯ menu, and takes no room until an agent has changed something |
