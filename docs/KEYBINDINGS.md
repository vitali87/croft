...
| Agent lanes (no chord) | Which foreground processes are coding agents and what their prompts look like, from `~/.config/croft/agents.json` (palette: "Preferences: Open Agent Lanes (JSON)", reloaded on save): built-in rows for claude, codex, aider, gemini; a row named like a built-in replaces it |
