...
| `Cmd+Opt+Up` / `Cmd+Opt+Down` (Linux: `Ctrl+Alt`) | Jump to the previous / next command prompt (VS Code's command navigation; the jumped-to prompt parks at the top of the pane). Powered by OSC 133 shell integration: croft auto-installs zsh hooks via a `ZDOTDIR` shim that sources your own dotfiles unchanged; opt out with `CROFT_SHELL_INTEGRATION=0` |
| Click a gutter dot on the pane's left border | Each finished command gets a VS Code-style decoration dot at its prompt row: blue for success, red for a non-zero exit. Clicking it shows the exit code and how long the command ran in the status bar. A command over 10s finishing in a pane you're not focused on announces itself in the status bar on its own |
