...
| Mouse drag | Select text, pinned to the scrollback content; drag past an edge to auto-scroll through history. Inside a full-screen app that scrolls by repainting (Claude Code, a pager), the highlight follows its text across the app's own scrolling — it hides while the text is scrolled out of the app's view (copy still yields it) and reappears with it — and a drag held past an edge forwards wheel ticks so the app scrolls under the drag |
