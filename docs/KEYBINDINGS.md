...
So the lifecycle is **Available → Add → Installed → Enable → (provision on first use)**, each a distinct visible state, and it is reversible: an INSTALLED catalog extension shows a **trash button** (🗑) just left of its toggle — click it to uninstall (its manifest is removed and it drops back to AVAILABLE to re-add), or select the row and press `Delete`. croft only uninstalls what it added from the catalog; a manifest you hand-dropped is your own file, so it shows no trash button and is never deleted (disable it instead). Hover any row to get a tooltip explaining the toggle's state, the `+Add` affordance, or the trash button.
