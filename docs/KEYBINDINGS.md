...
| `Cmd+K` `D` | Open the pane's entire scrollback in a rendered log tab named after the pane, colours included: the dump is written under `~/.cache/croft/scrollback/` and opens through the same ANSI log view a colour log file gets, so find and selection/copy work on it and the theme's palette paints it. A pane whose output carries no colour opens a plain scratch tab instead, where vim mode, block selection and save-to-file also apply |
