...
| `Cmd+C` / `Ctrl+Shift+c` | Copy the terminal's current selection. On a remote session the text goes to the clipboard of the machine you are typing at, not the remote box's. Inside a program that tracks the mouse (any full-screen TUI) a plain drag belongs to that program, so **Shift+drag** is what selects there |
