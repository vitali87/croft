...
* **Left pane (sidebar):** Explorer with multi-select, cut / copy / paste, drag-and-drop moves, and VS Code style icons. Below the file tree sits a collapsible **OUTLINE** section: the active editor's symbol tree (functions, classes, fields, etc.), indented by nesting with kind icons, that highlights the symbol the caret is in and jumps the editor to a symbol on click. It paints instantly from tree-sitter on file open and is then refined by the language server's richer symbols when they arrive, so it never waits on a cold server. A long outline grows to at most half the tree+outline region (a 50/50 split); past that both the file tree and the outline scroll independently under the mouse wheel. Search and a Remote (SSH) explorer swap in via the activity bar.
