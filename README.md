...
| Node.js + npm (optional) | TypeScript / JavaScript LSP. croft auto-installs the `vtsls` server into `~/.croft/servers` the first time you open a `.ts`/`.tsx`/`.js` file. It finds `node` even when a version manager keeps it off croft's PATH: first by reading the on-disk layout directly (highest nvm `versions/node`, Volta, Homebrew, `/usr/local`), and as a universal fallback by asking your login shell where `node` resolves — run in a detached session (`setsid`) so it can never touch croft's terminal. That covers nvm / fnm / asdf / volta / custom setups, for both the install and running the server. Other language servers (`basedpyright`, `ruff`, `ty`, `rust-analyzer`, `gopls`) are used from your PATH if present. |
