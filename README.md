...
* **Activity bar:** the icon strip down the far left. View icons (Explorer, Search, Source Control, Remote, Run and Debug) stack from the top; a settings gear sits at the bottom (VS Code's "Manage" button). Clicking the gear opens a menu with **Color Theme**, a picker that switches between the **Croft Black** (`#000000`, the default) and **Croft Dark (Blue)** (`#1e222e`) themes. Under Croft Black the focused pane is outlined with the orange→green gradient border from the welcome screen's activity box, for every pane that can take focus (editor, terminal, and all five sidebar views: Explorer, Search, Source Control, Run and Debug, Remote Explorer). The Explorer's selected and marked rows (and the Search results selection, and the Remote Explorer's selected SSH host) fill with the same muted dark-teal (`#264f4a`) as the menus instead of the legacy bright-blue, the inner input focus rings, cursors, chevron, and magnifier of the Search and Source Control panels switch to a muted brand teal (`#4fb1a6`), and every popup that appears (right-click menus, the Color Theme picker, the New File / Rename prompt, Go to File, the zoxide jump, Find, the shortcuts modal, the connect dialog, LSP completion and hover) wears that same gradient border with the dark-teal selection bar; the bottom status bar drops to a near-black (`#181818`) seam. Croft Dark keeps the solid-blue focus highlight, blue popup accents, blue tree selection, and navy status bar. The choice persists across launches in `~/.config/croft/config.json`.
...
* LSP semantic-token overlay over the tree-sitter base (`textDocument/semanticTokens/full`): for any language whose server advertises a `semanticTokensProvider`, resolved symbols are recolored project-aware, so a symbol resolved across modules (an imported name, a type-dependent attribute) is colored the same as at its declaration, which syntax alone cannot know. Tree-sitter paints instantly and remains the fallback; semantic tokens refine on top once the server replies and re-request, debounced, after edits. For Python, TypeScript, TSX, JavaScript, and Rust the tree-sitter base resolves *local* scope itself via a locals query, so a parameter (and the locals it can track) is colored the same at its body references as at its declaration the instant the file opens, the way Helix and Zed do, without waiting on the language server; the semantic overlay then agrees on the same color, so there is no grey-to-color flicker on a cold file where the language server (e.g. `ty`, `vtsls`, or `rust-analyzer`, whose type-aware tokens land only after its crate-graph analysis finishes seconds later) takes a while to answer its first request. Because tree-sitter-rust ships no `(identifier)` catch-all, croft prepends one so the locals resolver has a capture to emit on, while the bundled specific rules still win, so function calls, constants, and types keep their colors. For Rust, croft also paints the tokens that otherwise recolor when rust-analyzer's overlay lands a second or two after open: module / namespace path segments (`std::collections::`, lowercase scoped paths) and enum-variant declarations (`enum E { A, B }`) are colored in the tree-sitter base to match the server, and `use` declarations are colored by resolved kind — imported type / trait / struct names (`Context`, `Result` in `use anyhow::{Context, Result}`) as types and brace-list module heads (`anyhow` in `anyhow::{…}`) as namespaces — so the whole import block at the top of a file paints its final color on the first frame instead of flickering grey/blue-to-yellow when the overlay arrives. The Python rule of "CamelCase is a class" cannot be applied blanket in Rust because enum variants (`Some`, `Ok`, `Err`, `Color::Red`) are CamelCase too yet are not types, and syntax cannot tell a variant from a struct in expression position; coloring `use` paths is the unambiguous slice (a variant is never written in a `use` statement). The residual that still refines when the overlay arrives — capitalized scoped *uses* (`E::A` versus `module::Type`) and CamelCase names inside macro bodies (`vec![Foo { … }]`, where tree-sitter sees only unparsed tokens) — is syntactically undecidable and is covered by the disk cache on every reopen. For Python, the same principle closes the largest cold-open mismatch on an import-heavy file: module names in `import` / `from ... import` statements are colored as namespaces, CamelCase identifiers are colored as classes (matching `ty`'s `class` rather than the bundled query's `@constructor`, and excluding `ALL_CAPS` names so module-level constants stay distinct), and a module reference in the body (`pydantic` in `pydantic.BaseModel`) resolves through the locals query to inherit the namespace color, all instantly from syntax. This brings the tree-sitter base to ~84% exact agreement with `ty` at open, so the imports and types that used to flash grey now paint their final color on the first frame. When several servers are available, croft prefers a range-capable, incremental highlighter (e.g. Astral's `ty`, which answers in tens of milliseconds even on a huge cold workspace) over a full-only one that first pays a slow whole-tree enumeration, so colors appear effectively instantly without giving up `basedpyright` for completion. The remaining tokens are genuinely type-dependent — whether a builtin like `list` is a class or `len` a function, whether `obj.attr` is a plain variable or a property, whether a name like `Any` is a value or a type — and syntax cannot decide them; rather than encode the server's per-name choices (fragile, and coupled to a specific server version), croft persists each whole-document semantic-token batch to a content-keyed disk cache (`~/.croft/sem-cache`). Because the server's output is deterministic for identical content, the next open of that exact file (a new tab, after a restart, the next day) applies the cached batch synchronously and paints the server's colors on the first frame; the live reply still arrives behind it and, being identical, repaints to the same pixels, so there is no grey-to-color snap. The cache is content-addressed, so an edit simply produces a different key (the prior entry is never consulted for changed text, and the new text is re-cached once the server re-replies) and there is no invalidation to get wrong; it is best-effort and self-healing, a missing entry falls back to the tree-sitter base until the live reply lands, and the live `full` batch is always authoritative. Buffers are split into lines on `\r\n`, lone `\r`, and `\n` (matching the LSP / VS Code line model) so token positions stay aligned with the server on files with mixed line endings.
...
* LSP diagnostics (`textDocument/publishDiagnostics`): errors, warnings, and hints the language server pushes are underlined in the editor where they occur, coloured by severity (red error, amber warning, blue info / hint) using the terminal's separate underline colour so the glyph's syntax colour is left untouched. Each server publishes the complete set for a file, so croft keeps the latest set per `(file, server)` pair and layers them — `ty` and `ruff` on one Python file both show — and an empty "all clear" republish erases the underlines. Positions arrive as LSP UTF-16 offsets and are decoded against the editor's buffer (so astral-plane characters stay aligned); the store survives tab switches, so re-focusing a file shows its diagnostics again without waiting for the next server push. Hovering a squiggle surfaces its message in the hover popup (above any type / signature hover), so the underline tells you *where* and the hover tells you *what*.
