...
| Node.js + npm (optional) | TypeScript / JavaScript LSP. croft auto-installs the `vtsls` server into `~/.croft/servers` the first time you open a `.ts`/`.tsx`/`.js` file. It finds `node` even when a version manager keeps it off croft's PATH by reading the on-disk layout directly (highest nvm `versions/node`, Volta, Homebrew, `/usr/local`), and uses it for both the install and running the server. If your node lives somewhere else, put it on PATH (e.g. `brew install node`). Other language servers (`basedpyright`, `ruff`, `ty`, `rust-analyzer`, `gopls`) are used from your PATH if present. |
