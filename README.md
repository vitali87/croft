...
* **Activity bar:** the icon strip down the far left. View icons (Explorer, Search, Source Control, Remote, Run and Debug) stack from the top; a settings gear sits at the bottom (VS Code's "Manage" button). Clicking the gear opens a menu with **Color Theme**, a picker that switches between the **Croft Black** (`#000000`, the default) and **Croft Dark (Blue)** (`#1e222e`) themes. Under Croft Black the focused pane (editor, terminal, or explorer) is outlined with the orange→green gradient border from the welcome screen's activity box, and every popup that appears (right-click menus, the Color Theme picker, the New File / Rename prompt, Go to File, the zoxide jump, Find, the shortcuts modal, the connect dialog, LSP completion and hover) wears that same gradient border with a muted dark-teal (`#264f4a`) selection bar instead of the legacy bright-blue; the bottom status bar drops to a near-black (`#181818`) seam. Croft Dark keeps the solid-blue focus highlight, blue popup accents, and navy status bar. The choice persists across launches in `~/.config/croft/config.json`.
