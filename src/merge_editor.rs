...
mod tests {
    ...
    #[test]
    fn editing_the_line_after_an_empty_region_leaves_it_unresolved() {
        // No base side (markers without diff3): the Result holds nothing for
        // the conflict, so its region is empty and sits at `b`'s row.
        let base = lines(&["a", "b"]);
        let ours = lines(&["a", "o", "b"]);
        let theirs = lines(&["a", "t", "b"]);
        let (mut view, result) = MergeView::new(base, ours, theirs, false);
        assert_eq!(view.conflicts.len(), 1);
        assert_eq!(view.conflicts[0].result_len, 0);
        let row = view.conflicts[0].result_start;
        // Typing into `b` changes no line count.
        view.sync_with_buffer(result.len(), 1, row);
