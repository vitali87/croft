...
/// The gutter menu's "Add Logpoint" opens the popup prompt; committing a
/// message creates the breakpoint with a `logMessage` attached, so the
/// adapter prints instead of pausing (VS Code's logpoint).
#[test]
fn add_logpoint_attaches_a_log_message_to_the_line() {
    use crossterm::event::{MouseButton, MouseEventKind};
    let tmp = tempfile::tempdir().unwrap();
    let f = tmp.path().join("prog.py");
    std::fs::write(&f, "l1\nl2\nl3\nl4\nl5\n").unwrap();
    ...
    let idx = app
        ...
        .items
        .iter()
        .position(|e| menu_label(e) == "Add Logpoint")
        .expect("menu must offer Add Logpoint");
    app.context_menu.as_mut().unwrap().selected = idx;
    app.handle_menu_key(key(KeyCode::Enter, KeyModifiers::NONE));

    let prompt = app.prompt.as_ref().expect("logpoint must open a popup");
    assert!(
        matches!(prompt.kind, PromptKind::Logpoint { line: 3, .. }),
        "popup must target the clicked line (3)"
    );
    for c in "x is {x}".chars() {
        app.handle_key(key(KeyCode::Char(c), KeyModifiers::NONE))
            .unwrap();
    }
    app.handle_key(key(KeyCode::Enter, KeyModifiers::NONE))
    ...
    assert_eq!(
        app.editor.breakpoint_lines(&f),
        vec![3u32],
        "committing a logpoint must create the breakpoint on line 3"
    );
    let lines = app.editor.breakpoints.get(&f).unwrap();
    let specs = app.editor.source_breakpoints(&f, lines);
    assert_eq!(
        specs[0].log_message.as_deref(),
        Some("x is {x}"),
        "the typed message must ride the breakpoint as logMessage"
    );
    assert_eq!(
        specs[0].condition, None,
        "a logpoint sets no condition of its own"
    );
}
...
/// The external-change sweep has to reach every editor GROUP, not just the
/// focused one. The poll stats each open tab exactly once and re-baselines its
/// stamp, so a change it reports and the sweep then misses is burned: no later
/// tick sees it again and the split pane keeps stale text forever.
#[test]
fn an_external_write_reloads_a_tab_in_an_inactive_split_group() {
    let tmp = tempfile::tempdir().unwrap();
    let side = tmp.path().join("side.txt");
    let front = tmp.path().join("front.txt");
    std::fs::write(&side, "old\n").unwrap();
    std::fs::write(&front, "front\n").unwrap();
    let mut app = App::new(tmp.path().to_path_buf()).unwrap();
    app.editor.open_pinned(&side).unwrap();
    app.split_editor(); // side.txt is now open in BOTH groups; right is focused
    app.editor.open_pinned(&front).unwrap(); // focused group moves to front.txt
    app.sync_open_file_poll_mtime();
    app.fs_watch.disable();
    app.fs_watch.force_poll_due();
    let _ = app.drain_fs_events();

    std::fs::write(&side, "new content\n").unwrap();
    app.fs_watch.force_poll_due();
    let _ = app.drain_fs_events();
    ...
    let stale: Vec<String> = app
        ...
        .into_iter()
        .flat_map(|g| g.editors.iter())
        .filter(|e| e.path.as_deref() == Some(side.as_path()))
        .map(|e| e.lines.join("\n"))
        .collect();
    assert!(
        !stale.is_empty(),
        "the inactive group still holds a tab on side.txt"
    );
    for text in stale {
        assert_eq!(
            text, "new content",
            "a tab in an inactive split group must reload too"
        );
    }
}
...
fn toggle_render_whitespace_cycles_modes_across_every_editor() {
    ...
    app.editor
        .open_text_buffer(std::path::Path::new("later.txt"), "c d")
        .unwrap();
    app.sync_focus_flags();
    assert!(
