...
pub const RELEASE_NOTES: &[ReleaseNote] = &[ReleaseNote {
    ...
    summary: "croft plot: pipe numbers, CSV/TSV or JSON lines into an inline chart. `seq 1 100 | awk '{print sin($1/10)}' | croft plot` draws a line chart in the pane on iTerm2, Kitty and sixel terminals and a braille chart everywhere else; --type bar labels bars from the first column, spark fits one row, hist bins the values. Headers are detected, --x/--y pick columns, and 10k rows render in a blink.",
}];
